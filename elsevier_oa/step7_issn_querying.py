"""query the Elsevier API to gather journal-level metadata"""
## get ISSN info
import argparse
import json
import os
...
def load_secrets() -> None:
    ...
    try:
        ...
        print(e, env_path, "file read error")
...
def format_pageinfo(metadata: Dict[str, Any]) -> str:
    """
    Formats the page range into a readable string from a metadata dictionary.

    This function attempts to extract 'firstpage' and 'lastpage' from the 
    provided metadata dictionary. If either value is missing or None, it 
    returns 'Unknown pages'. Otherwise, it returns a formatted string 
    representing the page range (e.g., "p. 10-20").

    Args:
        metadata (Dict[str, Any]): A dictionary containing metadata, 
            expected to have 'firstpage' and 'lastpage' keys.

    Returns:
        str: A formatted string such as "p. 1-10" or "Unknown pages".
    """
    firstpage = metadata.get('firstpage')
    lastpage = metadata.get('lastpage')

    if firstpage is None or lastpage is None:
        return 'Unknown pages'

    return f"p. {firstpage}-{lastpage}"
...
def refactor_metas(json_file: Path) -> Tuple[str, Dict[str, str]]:
    ...
    Returns:
        Tuple[str, Dict[str, str]]: A tuple containing:
            - doc_id (str): The stem of the filename.
    ...
    ## page info
    pageinfo = format_pageinfo(metadata)
...
def make_dict(folder_with_jsons: str) -> List[str]:
    ...
    # Convert list of tuples [(doc_id, dict), ...] into a single dictionary
    output_dict: Dict[str, Dict[str, str]] = dict(results)
...
def _extract_metadata_details(data: Dict[str, Any]) -> Tuple[str, str]:
    """
    Navigates the nested JSON structure to extract subject area and title.
    
    Args:
        data (Dict): The raw JSON response from the Elsevier API.

    Returns:
        Tuple[str, str]: (subject_code, title)
    """
    msg = data.get("serial-metadata-response")
    if not msg:
        raise ValueError("serial_error")

    try:
        # msg['entry'][0] is the standard structure for Elsevier entries
        entry = msg['entry'][0]

        # Extract subject code with a fallback
        try:
            subj_code = entry['subject-area'][0]['@abbrev']
        except (KeyError, IndexError, TypeError):
            subj_code = "UNK"

        # Extract title with a fallback
        try:
            title = entry['dc:title']
        except (KeyError, IndexError, TypeError):
            title = "UNK"

        return subj_code, title

    except Exception:
        # This catches any unexpected structure issues within the 'entry' list
        raise ValueError("Exception_error")

def make_error_res(status: str, meta_msg: str) -> Dict[str, str]:
    """Standardizes the error response dictionary."""
    return {
        "status_code": status,
...
def process_one_issn(issn: str) -> Dict[str, str]:
    ...
    This refactored version uses a single exit point for the 'success' path 
    and handles errors via a try-except block to satisfy Pylint requirements 
    while maintaining clear logic flow.

    Args:
        issn (str): The International Standard Serial Number to query.

    Returns:
        Dict[str, str]: A dictionary containing status, metadata, subject, and title.
    """
    ...
    try:
        ...
        # Extract and return the success dictionary
        subj_code, title = _extract_metadata_details(data)
        return {
        ...
        # Handles the custom errors raised by _extract_metadata_details
        # or cases where the JSON structure is missing keys.
        # We know the status code is 200 because we passed the first check.
        return make_error_res("200", str(ve))

    except requests.RequestException:
        # Handles timeouts, DNS issues, etc.
...
def run_issn_querying(
    ...
    print(f"Running queries on {len(tidy_issns)} issns with batch == {buffer} and delay = {delay}")

    for num, issn in tqdm(enumerate(tidy_issns, start=1), desc="Querying ISSNs"):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="0",
        help="Skip flag. Usage : 1 will skip JSON parse and load from issn_infile\
        2 will skip API querying."
    )
    parser.add_argument(
        "--issn_infile",
        type=str,
