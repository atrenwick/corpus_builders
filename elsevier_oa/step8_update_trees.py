...
def init_worker(
    ...
    Returns:
        None
    """
    global article_metas_dict, issn_title_dict
    ...
    article_metas_dict, issn_title_dict = load_dicts(
...
def load_dicts(
    ...
    try:
        ...
        if not isinstance(issn_title_dict, dict):
            print(f"Error: File {issn_title_map_path} did not load as a dictionary.")
            sys.exit(1)
...
def reshape_tree_body(input_tree: etree._Element) -> None:
    ...
    for pblock in input_tree.findall(".//p"):
        ...
        if p_type != current_type:
            current_div = etree.Element("div", type=str(p_type) if p_type is not None else "unknown")
...
def process_one_file(file_path: str, output_dir: str) -> None:
    ...
    docId = input_tree.xpath(".//metas/@docId")[0]
    ...
    new_header_element = make_full_header(docId, article_metas_dict, issn_title_dict)
...
def main(
    ...
    Args:
        source_dir (str): Directory containing the source XML files.
        output_dir (str): Base directory for saved XML outputs.
        article_metas_dict_file_path (str): Path to the large metadata JSON.
        issn_title_dict_file_path (str): Path to the ISSN mapping JSON.
        sibling (bool): A flag for sibling-specific logic (if applicable).
        n_procs (int, optional): Maximum number of processes to use. 
    ...
    xml_files = make_file_lists(source_dir, sibling, extension)
    ...
    with ProcessPoolExecutor(
        ...
        initargs=(article_metas_dict_file_path, issn_title_dict_file_path)
    ) as ex:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sibling", 
        help="Run in sibling mode : specify a folder of xml files and run the process for all its siblings", 
        required=False, 
        default=False, 
        action="store_true"
    )
